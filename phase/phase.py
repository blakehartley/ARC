...
for arg in range(int(sys.argv[1]), (int(sys.argv[2]))):
	...
	hist, bin_edges = np.histogram(arr_energy.flatten(), bins=1000)
	plt.plot(bin_edges[0:-1], hist, label="Hubble Cooling, Adaptive dt")
	
	'''output_1 = np.load("output_nocool_adapt.npy")
	hist, bin_edges = output_1[0], output_1[1]
	plt.plot(bin_edges, hist, label="No Cooling, Adaptive dt")'''
	...
	plt.close()
	output = np.array([hist, bin_edges[0:-1]])
	np.save("output_cool_adapt", output)
	"""
