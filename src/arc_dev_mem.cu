...
void initialize_rays (	const source* source_dev, Ray* RecvBuf, Ray* SendBuf, int* nRays0, Ray* RayDat0_dev, Ray* RayDat_dev, Ray* RayBuf_dev,
						int* PartInfo, int* ndBuf, Domain domain)
{
	// JET
	srand(PartInfo[0]);
	// JET
	...
	if(PartInfo[3] == 0)
	{
		...
		for(int i=0; i<partNumAdd; i++)
		{
			...
			if(jetOn == 0)
			{
				...
				{
					//int index = i*nPix + j;
					RecvBuf[index].set_dom(domain.get_id());
					RecvBuf[index].set_part(PartInfo[2]);
					RecvBuf[index].set_pix(j, ord);
					...
					RecvBuf[index].set_flux(flux_init);
					...
					RecvBuf[index].set_position(position, 0.0, direction);
		...
		//printf("Domain %d is starting %d rays\n", domain.get_id(), nRays0[0]);
	}
	
	//printf("B %d\t%d\t%d\n", PartInfo[1], PartInfo[2], partNumAdd);
	
	// Tracing rays from the buffer, first check if there are any rays to 
	if(PartInfo[3] == 1)
	{
		nRays0[0] = 0;
		for(int i=0; i<8; i++)
	...
	{	
		...
		for(int i=0; i<8; i++)
		{
			CUDA_CHECK( cudaMemcpy(	RayDat0_dev + d_dat,
									RecvBuf + d_buf,
									ndBuf[i]*sizeR, 
									cudaMemcpyHostToDevice) );
	...
	//printf("Quick buffer check -> %d")
	// Reset buffer counts
	//memset(ndBuf, 0, 8*sizeof(int));

	// At this point, there are nRays0 = nRays rays in RayDat0_dev, and 0 in all of the ndBuf.
}
...
void rad(	float* DensArray_dev, float* x_NArray_dev, const source* source_dev, float* FluxArray_dev, float* dEArray_dev,
			Ray* RecvBuf, Ray* SendBuf, int* nRays0, Ray* RayDat0_dev, Ray* RayDat_dev, Ray* RayBuf_dev,
			int* PartInfo, int* ndBuf, float L, float a, double *local_vars, float* nfSback, Domain domain, float dt_ion)
{
	int dim = domain.get_dim();
	//printf("Domain %d is processing %d rays\n", domain.get_id(), nRays0[0]);
	/*int nDevices;
	...
	// Temporary variable for total number of currently active rays, distinct from nRays0
	int nRays=0;
	...
	for(;nRays0[0]>0;)
	{
		...
		int grid = (int) sqrt(nRays0[0]/256) + 1;
		dim3 blocksPG(grid, grid, 1);
		//dim3 blocksPG(16, 16, 1);
		...
		rayTraceKernel<<<blocksPG,threadsPB>>>(	DensArray_dev, x_NArray_dev, 
												...
												RayDat0_dev, nRays_dev, nRays0[0],
		...
		CUDA_CHECK( cudaMemcpy(&nRays, nRays_dev, sizeof(int), cudaMemcpyDeviceToHost) );
		//CUDA_CHECK( cudaMemset(nRays_dev, 0, sizeof(int)) );
		//printf("Domain %d\tnRays0 = %d\tnRays = %d, temp_nrays = %d\n", domain.get_id(), nRays0[0], nRays, 4*temp_nrays);
		//nRays = nRays*4;
		
		int information = 1000000*domain.get_id() + nRays0[0];
		...
		CUDA_CHECK( cudaMemcpy(RayDat0_dev, RayDat_dev, nRays*sizeR, cudaMemcpyDeviceToDevice) );
		...
		// Reset counter
		nRays0[0] = nRays;
		// Output for checking device buffers
		if(domain.get_id()==1)
		{
			int ndTemp[8];
	...
	CUDA_CHECK( cudaMemcpy(	ndBuf, NumBuf_dev,
							8*sizeof(int), cudaMemcpyDeviceToHost) );
	CUDA_CHECK( cudaMemcpy( SendBuf, RayBuf_dev,
