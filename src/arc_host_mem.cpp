...
int array_diagnostic(float *array, int n, bool print=true)
{
	float min=array[0];
	float max=array[0];
	float mean=0;
	...
	mean = mean/n;
	if(print)
		printf("Diagnose %e < %e < %e, nan = %d\n", min, mean, max, nan_elements);
...
int main(int argc, char** argv)
{
	...
	// These arrays store the rays that are being sent and received, respectively.
	//Could probably be 8-1 but that requires math and coordination I don't want to work out right now.
	Ray *SendBuf = new Ray[8*NUM_BUF];
	Ray *RecvBuf = new Ray[8*NUM_BUF];

	size_t sizeR = sizeof(SendBuf[0]);
	
	// These arrays are where the rays are stored on the device, representing the beginning and end points.
	__device__ Ray *RayDat0_dev;
	__device__ Ray *RayDat_dev;
	CUDA_CHECK( cudaMalloc((void **)&RayDat0_dev, 8*NUM_BUF*sizeR) );
	CUDA_CHECK( cudaMalloc((void **)&RayDat_dev, 8*NUM_BUF*sizeR) );

	// Third array... might be necessary?
	__device__ Ray *RayBuf_dev;
	...
	conservation << "x_HeII\t\tGam_He*T\tAlp_He*T\tFlux/S0\t\tescaped" << endl;
	conservation.close();
...
//					cout << slice << "\t" << state << endl;
		...
		while(nPartRem > 0)
		{
			...
			// Initialize rays and run the ray tracing routine once
			// End result should be that the SendBuf is filled 
			MPI_CHECK(MPI_Barrier(commGridDom));
			if(commRank==0) {dT0a=MPI_Wtime();}
			initialize_rays (	source_dev,
								RecvBuf, SendBuf, &nRays0,
								RayDat0_dev, RayDat_dev, RayBuf_dev,
								PartInfo, ndBuf, domain);
			
			MPI_CHECK(MPI_Barrier(commGridDom));
			if(domain.get_id() == 0)
				printf("First RAD run beginning with %d rays\n", nRays0);
			rad(	DensArray_dev, x_NArray_dev, source_dev, FluxArray_dev, dEArray_dev,
					SendBuf, RecvBuf, &nRays0, RayDat0_dev, RayDat0_dev, RayBuf_dev,
			...
			for(int j=0; j<MAX_LOOPS; j++)
			{
				...
				// At this point, all of the rays should be in RecvBuf
				MPI_CHECK(MPI_Barrier(commGridDom));
				if(domain.get_id() == 1)
				{
					printf("Recv totals of ");
					for(int k=0;k<8;k++)
					{
						printf("%d ", ndBuf[k]);
					}
				...
				/*CUDA_CHECK( cudaMemcpy(x_NArray, x_NArray_dev, nSizeBytes*SPECIES, cudaMemcpyDeviceToHost) );
				if(array_diagnostic(x_NArray, nSize*SPECIES, false)>0)
				{
					printf("3. Found problem at %d (", commRank);
					array_diagnostic(x_NArray, nSize*SPECIES, true);
					for(int i=0; i<8; i++)
						printf("%d ", ndBuf[i]);
					printf(") loop %d\n", j);
					return 0;
...
//		cout << "Process #" << commRank << " has finished the rad kernel." << endl;
	...
	CUDA_CHECK( cudaFree(RayDat_dev) );
