...
data = np.genfromtxt("./profiles/conservation_1.0.dat", skip_header=1).T
label = r'dt = 1.0 Myr'
data1 = np.genfromtxt("./profiles/conservation_0.5.dat", skip_header=1).T
label1 = r'dt = 0.5 Myr'
...
axs[0].plot(data[5], data[4], c='C0', label=label)
axs[0].plot(data1[5], data1[4], c='C1', label=label1)

axs[0].legend(loc=1)
axs[0].set_title("Ratios vs ionized fraction")
#axs[0].semilogy()
axs[0].set_ylabel('Ratio')

axs[1].plot(data[5], data[8], c='C0', label=label)
axs[1].plot(data1[5], data1[8], c='C1', label=label1)

#axs[1].legend(loc=1)
#axs[1].semilogy()
axs[1].set_xlabel('x_HII')
axs[1].set_ylabel('Flux/S0')

fig.tight_layout()
plt.savefig("./profiles/comparison.png")
plt.show()
sys.exit()
##################################################
