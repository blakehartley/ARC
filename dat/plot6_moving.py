...
nrot = 5
#for arg in np.arange(slice0,151):
for arg in range(int(sys.argv[3])+slice0, (int(sys.argv[2]))):
	...
	shift = (arg-50)*16
	arr_h = np.roll(arr_h, shift, axis=0)
	distance = shift
	...
	print(slist[0:5])
	shift = shift % DIMX
	slist[:,shift_dim] = slist[:,shift_dim] + shift
	slist[:,shift_dim][slist[:,shift_dim] > DIMX] = slist[:,shift_dim][slist[:,shift_dim] > DIMX] - DIMX
	print(slist[0:5])
	...
	for i in cam.iter_rotate(DTheta, NTheta):
		...
		#vec = np.array([0.7071*dx, 0.7071*dy, 0.5*(1.0 - 0/400.0)])
		#cam.set_position(orig + 1.75*vec, north_vector = north_vector)

		north_vector = (0, 0, 1)
		vec = np.array([0.7071*dx, 0.7071*dy, 0])
		cam.set_position(orig + 1.75*vec, north_vector = north_vector)
		...
		sc.render()
		sc.save('./figures/moving_%03d.png' % frame, sigma_clip=1)
		j+=1
