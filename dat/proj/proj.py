...
for arg in range(int(sys.argv[1]), (int(sys.argv[2]))):
	...
	slice = arr_h[slice_num,:,:]
	for i in range(0, DIMX):
		for j in range(0, DIMX):
			threshold = 0.9
			if(slice[i, j] > threshold and tracking_array[0, i, j] > 30):
				tracking_array[0, i, j] = sim_redshift(arg)
	
	thresholds = [0.1, 0.5, 0.9]
	mean = np.mean(arr_h[38:62,:,:], axis=0)
	...
	for threshold in [0.1, 0.5, 0.9]:
		for i in range(0, DIMX):
			for j in range(0, DIMX):
				...
				if(mean[i, j] > threshold and tracking_array[ind, i, j] > 30):
					tracking_array[ind, i, j] = sim_redshift(arg)
		ind = ind + 1
	print("Checked array {}".format(arg))

tracking_array[tracking_array>30] = np.min(tracking_array)

np.save("proj.npy", tracking_array)
