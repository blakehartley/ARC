...
for arg in range(int(sys.argv[1]), (int(sys.argv[2]))):
	fold = "./hydrogen/"
	arr_h = np.fromfile(fold+"xhi%03d.bin" % arg, dtype=np.float32)
	...
	fold = "./helium/"
	arr_he = np.fromfile(fold+"xhei%03d.bin" % arg, dtype=np.float32)
	arr_he = 1.0 - arr_he
	...
	arr_n = np.fromfile(fold + "GridDensities_256_%03d.bin" % int(arg/5), dtype=np.float32)
